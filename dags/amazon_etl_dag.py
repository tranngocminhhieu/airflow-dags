...
@dag(
    ...
    description='Extract Transform Load data from Amazon to MySQL database',
    schedule='@daily',
...
def amazon_books_etl():
    ...
    def get_amazon_data_books(num_books=50, max_pages=10, ti=None):
        ...
        print(f'[EXTRACT] Amazone book data successfully saved at {raw_path}')
        ...
        # Clean up non-breacking spaces and format neatly
    ...
    def transform_amazon_books(raw_file:str):
        ...
        if 'Price' in df.columns:
            ...
            print('[TRANSFORM] No price column - filling with None]')
            df['Price $'] = None
        ...
        if 'Rating' in df.columns:
            ...
            print('[TRANSFORM] No rating column - filling with None')
            df['Rating'] = None


        # --- Validation: drop rows where BOTH fields fields (optional)
    ...
    def load_to_mysql(transformed_file:str):
        ...
        for _, row in df.iterrows():
            try:
                cursor.execute(insert_query, (row['Title'], row['Author'], row['Price($)'], row['Rating']))
        ...
        print(f'[LOAD] Table {table_name} successfully loaded with {len(df)} rows')
